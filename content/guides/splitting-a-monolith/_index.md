...
demonolith split refactor      # the code split — no credentials, no state touched
  split refactor map           #   analyze → write the map (the file you review)
  split refactor run           #   execute the map: write the new module directories
  split refactor validate      #   ask the engine whether it accepts what was written
  split refactor diff          #   CI gate: output on disk still matches the source

demonolith split migrate       # the state migration
  split migrate map            #   pull read-only, back up, split into local state copies
  split migrate prove          #   prove the split changes nothing (plans over the local copies)
  split migrate run            #   push each module's state to its new backend (guarded, never forced)
  split migrate verify         #   judge the result against the real backends
```

Everything hinges on the **map** (`demonolith-refactor-map.yaml`): a reviewable file recording which block goes to which module, every state move, the values that will flow between modules, and where each module's state will live. `split refactor map` writes it, you review it, and every later step executes or checks against it.
